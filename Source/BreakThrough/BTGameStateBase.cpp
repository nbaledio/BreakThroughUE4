...
void ABTGameStateBase::SpawnCharacters()
{
    if (CharacterBlueprints.Num() > 0)
    {
        ...
        gs.Player[0]->CurrentState.Position = FVector2D(-80, gs.Player[0]->GetActorLocation().Z);
        gs.Player[1]->CurrentState.Position = FVector2D(80, gs.Player[1]->GetActorLocation().Z);
